...
for (var i = 0; i < currArmNum; i++) {
    ...
    if (armMoving[i] < 0) {
        ...
        if (d > armTriggerDist and d > largestD and armsMoving < maxArmsMoving) {
            largestD = d;
            largestInd = i;
        }
    } else {
...
if (largestInd >= 0) {
    ...
    var prev = armPrevPos[i];
    armMoving[i] = 0;
    armSpeed[i] = (0.1 * largestD) / armTriggerDist;
    armsMoving++;
    var target = armTarget[i];
    ...
    var overshoot = armOvershoot[i];
    overshoot[@ 0] = (0.5 * armTriggerDist * (target[0] - p[0])) / largestD;
    overshoot[@ 1] = (0.5 * armTriggerDist * (target[1] - p[1])) / largestD;
}
